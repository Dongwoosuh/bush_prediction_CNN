...
class build(nn.Module):
    ...
    def __init__(self,num_DV):
        ...
        self.fc = nn.Sequential(
            ...
            nn.SiLU(),
            nn.Linear(in_features=self.start_ch, out_features=self.start_ch * 2),        # (batch_size, 2048)
            nn.SiLU(),
        ...
        self.conv5 = nn.Sequential(
            nn.ConvTranspose2d(self.start_ch//2, self.start_ch // 8, kernel_size=self.kernel_size, stride=self.stride, padding=self.padding_param),  # (batch_size, 256, 4, 4)
            nn.BatchNorm2d(self.start_ch // 8, momentum=BN_momentum),
            nn.SiLU(),
            nn.Dropout(dropout_rate),

            nn.ConvTranspose2d(self.start_ch // 8, self.start_ch // 16, kernel_size=self.kernel_size, stride=self.stride, padding=self.padding_param),  # (batch_size, 256, 6, 6)
            nn.BatchNorm2d(self.start_ch // 16, momentum=BN_momentum),
            nn.SiLU(),
            nn.Dropout(dropout_rate),

            # nn.ConvTranspose2d(self.start_ch // 16, self.start_ch // 16, kernel_size=self.kernel_size, stride=self.stride, padding=self.padding_param),  # (batch_size, 128, 13, 13)
            # nn.BatchNorm2d(self.start_ch // 16, momentum=BN_momentum),
            # nn.SiLU(),
        ...
        self.conv_last = nn.Sequential(
            nn.ConvTranspose2d(self.start_ch // 16, 1, kernel_size=4, stride=2, padding=1),  # (batch_size, 1, 32, 32)
            nn.Sigmoid()
            # nn.ReLU()
    ...
    def forward(self, input):
        ...
        x = x.view(-1, self.start_ch//2, 2, 2)  # (batch_size, 512, 2, 2)
